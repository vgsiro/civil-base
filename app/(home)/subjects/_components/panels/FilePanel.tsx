...
import MindMapPanel from '../content/MindMapPanel'
import FormulaPanel from '../content/FormulaPanel'
import PdfPreviewModal from '../pdf/PdfPreviewModal'
import type { Section, Pdf, Formula, LectureMindMap, MindMapNode, UploadQueueItem } from '../../../../_types'
    ...
    <div style={{ flex: 1, background: '#f8fafc', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: '10px 16px', borderBottom: `1px solid ${reorderMode ? '#fbbf24' : '#e2e8f0'}`, background: reorderMode ? '#fffbeb' : 'white', display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: '11px', fontWeight: '600', color: reorderMode ? '#92400e' : '#64748b', textTransform: 'uppercase', flex: 1 }}>
          {reorderMode ? t('home_panel_reorder_unsaved') : (selectedSection ? selectedSection.name : t('home_panel_files'))}
        </span>
        {reorderMode ? (
          <>
            ...
            <div key={p.id}
              ...
              {!selectMode && !reorderMode && renamingPdfId !== p.id && (
                <div style={{ display: 'flex', gap: '2px', flexShrink: 0 }}>
                  ...
                  {!isImage && (
                    <button onClick={e => onOpenPdfChat(e, p)} title={t('home_panel_ask_ai')}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#cbd5e1', padding: '4px', borderRadius: '6px' }}
                  ...
                  <button onClick={e => { e.stopPropagation(); p.file_url && window.open(p.file_url, '_blank') }} title={t('home_open_new_tab')}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#cbd5e1', padding: '4px', borderRadius: '6px' }}
                    onMouseEnter={e => (e.currentTarget.style.color = '#3b82f6')}
                    onMouseLeave={e => (e.currentTarget.style.color = '#cbd5e1')}>
