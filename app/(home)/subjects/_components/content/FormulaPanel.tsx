...
import type { Section, Formula } from '../../../../_types'
...
export default function FormulaPanel({
  ...
  async function saveFormula() {
    ...
    if (error) { alert(t('home_formula_save_failed') + error.message); return }
      ...
      <div style={{ padding: '10px 16px', borderBottom: '1px solid #e2e8f0', background: 'white', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <span style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase', flex: 1 }}>
          ...
          ? (editingFormula.isEditing
            ? (editingFormula.isNew ? t('home_formula_new') : t('home_formula_edit'))
      ...
      {bulkFormulaMode ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ padding: '6px 14px', background: '#fffbeb', borderBottom: '1px solid #fef3c7', fontSize: '11px', color: '#92400e', lineHeight: 1.6 }}>
            <strong>{t('home_formula_format_label')}</strong> {t('home_formula_format_1')} <code>---</code> {t('home_formula_format_2')} <code>$...$</code> {t('home_formula_format_3')} <code>$$...$$</code> {t('home_formula_format_4')}
          </div>
          <textarea
          ...
          {bulkLog.length > 0 && (
            <div style={{ padding: '10px 16px', borderTop: '1px solid #e2e8f0', background: '#f8fafc', fontSize: '12px', lineHeight: 1.8 }}>
              ...
              <button onClick={() => { setBulkFormulaMode(false); setBulkLog([]) }}
                style={{ marginTop: 8, padding: '3px 14px', borderRadius: 6, background: '#f59e0b', color: '#fff', border: 'none', cursor: 'pointer', fontSize: '12px' }}>{t('home_formula_done')}</button>
            </div>
          )}
        </div>
      ) : !editingFormula ? (
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
          {formulas.length === 0 && (
            <div style={{ textAlign: 'center', padding: '60px 20px', color: '#94a3b8' }}>
              ...
              <p>{t('home_formula_no_formulas')}</p>
              <p style={{ fontSize: '13px', marginTop: '8px' }}>{t('home_formula_new_hint')}</p>
            </div>
          )}
          {formulas.map(f => {
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <input value={formulaName} onChange={e => setFormulaName(e.target.value)}
            placeholder={t('home_formula_name_ph')}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '11px', color: '#94a3b8' }}>{t('home_formula_editor_hint')}</span>
          ...
          <p style={{ fontSize: '11px', color: '#94a3b8', margin: 0 }}>{t('home_formula_resize_hint')}</p>
        </div>
      ) : (
