...
import { ALL_HOLLOW_ROWS } from './data/index'
import type { SectionRow } from '../_shared/types'
import { HollowExportModal } from './ExportModal'
import {
  DetailGroup,
  CalcStep,
  Tex,
...
function SectionSearch({ current, onSelect }: { current: SectionRow | null; onSelect: (r: SectionRow) => void }) {
  ...
  const matches = useMemo(() => {
    ...
    return lq ? ALL_HOLLOW_ROWS.filter(r => r.designation.toLowerCase().includes(lq)).slice(0, 20) : []
  }, [q])
  ...
  return (
    <div ref={ref} style={{ position: 'relative', flex: 1, maxWidth: 320 }}>
      <input
        type="text" value={q}
        placeholder={current ? current.designation : t('bbhs_search_placeholder')}
    ...
    <div style={{ background: 'linear-gradient(135deg, #0c4a6e, #0369a1)', borderRadius: 10, padding: '14px 18px', color: '#fff', marginBottom: 20 }}>
      <div style={{ fontSize: 13, fontWeight: 800, marginBottom: 6 }}>
        ...
        <span style={{ marginLeft: 10, fontSize: 11, fontWeight: 700, background: 'rgba(255,255,255,0.2)', borderRadius: 4, padding: '2px 8px' }}>
          {t('bbhs_header_class')} {cs.sectionClass}{cs.isClass4 ? ` — ${t('bbhs_header_class4_note')}` : ''}
        </span>
      </div>
...
export function CrossSectionDetails({ row, grade, cs }: { row: SectionRow; grade: SteelGrade; cs: CrossSectionResult }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <DetailGroup title={t('bbhs_det_class_title')}>
        {cs.isCHS ? (
          <>
            <CalcStep
              label={t('bbhs_det_eps_label')}
      ...
      {!cs.isCHS && (
        <DetailGroup title={t('bbhs_det_lc_title')}>
          <CalcStep
            label=""
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 700, color: '#1e293b' }}>{t('bbhs_bend_title')} <Tex>{'M_{b,Rd}'}</Tex> (kNm)</span>
        <span style={{ fontSize: 10, color: '#94a3b8', marginLeft: 4 }}>{t('bbhs_bend_ref')}</span>
      </div>
      ...
      <div className="bb-table-wrap" style={{ overflowX: 'auto', borderRadius: 8, border: '1px solid #e2e8f0', marginBottom: 10 }}>
        <table style={{ borderCollapse: 'collapse', fontSize: 11 }}>
          <thead>
            <tr>
              <th style={{ ...TH, textAlign: 'left', minWidth: 52 }}>{t('bbhs_bend_c1_col')}</th>
      ...
      <div style={{ fontSize: 10, color: '#64748b', marginBottom: 8 }}>
        {t('bbhs_bend_lc_note')} (L<sub>c</sub> = {nf(cs.Lc_y, 2)} m)
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 11, color: '#0369a1', fontWeight: 600 }}>{t('bbhs_tab_details')}:</span>
      ...
      {showDetails && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <DetailGroup title={t('bbhs_det_ltb_title')}>
            <CalcStep
              label={t('bbhs_det_mcr_label')}
          ...
          <DetailGroup title={<><Tex>{'\\bar{\\lambda}_{LT}'}</Tex> — {t('bbhs_det_lam_lt_label')}</>}>
            <CalcStep
              label={t('bbhs_det_lam_lt_label')}
          ...
          <DetailGroup title={<><Tex>{'\\chi_{LT}'}</Tex> — {t('bbhs_det_chi_lt_label')}</>}>
            <CalcStep
              label={t('bbhs_det_phi_lt_label')}
              formula={`\\Phi_{LT} = 0.5\\left[1 + 0.34(\\bar{\\lambda}_{LT} - 0.4) + 0.75\\bar{\\lambda}_{LT}^2\\right] = ${nf(pt.phi, 4)}`}
            />
            <CalcStep
              label={t('bbhs_det_chi_lt_label')}
              formula={`\\chi_{LT} = ${nf(pt.chiLT, 4)}`}
            />
            <CalcStep
              label={t('bbhs_det_f_label')}
              formula={`\\chi_{LT,\\mathrm{mod}} = ${nf(pt.chiLTmod, 4)}`}
            />
            <CalcStep
              label={t('bbhs_det_mbrd_label')}
...
export function AxialBendingSection({ row, grade, mnRows, cs, showDetails, selN, onSelN }: {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        <div style={{ width: 3, height: 14, borderRadius: 2, background: ACCENT }} />
        <span style={{ fontSize: 12, fontWeight: 700, color: '#1e293b' }}>{t('bbhs_axbend_title')} <Tex>{'M_{N,Rd}'}</Tex> (kNm)</span>
        <span style={{ fontSize: 10, color: '#94a3b8', marginLeft: 4 }}>{t('bbhs_axbend_ref')}</span>
      </div>
      ...
      <div className="bb-table-wrap" style={{ overflowX: 'auto', borderRadius: 8, border: '1px solid #e2e8f0', marginBottom: 10 }}>
        <table style={{ borderCollapse: 'collapse', fontSize: 11 }}>
          ...
          <tbody>
            {/* Row 0 = MN,y,Rd; Row 1 = MN,z,Rd (skipped for SHS/CHS — same value) */}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: 11, color: '#64748b' }}>
          ...
          {!cs.isSHS && <span style={{ marginLeft: 8 }}><Tex>{'M_{N,z,Rd}'}</Tex> = <strong style={{ color: '#0369a1' }}>{r2(pt.MN_z_Rd)} kNm</strong></span>}
        </span>
      </div>

      {showDetails && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <DetailGroup title={t('bbhs_det_mnrd_title')}>
            <CalcStep
              label={t('bbhs_det_aw_label')}
              formula={`a_w = \\dfrac{A - 2bt}{A} \\le 0.5 = ${nf(pt.aw, 4)}`}
            />
            {!cs.isSHS && (
              <CalcStep
                label={t('bbhs_det_af_label')}
                formula={`a_f = \\dfrac{A - 2ht}{A} \\le 0.5 = ${nf(pt.af, 4)}`}
              />
            )}
            <CalcStep
              label={t('bbhs_det_mny_label')}
...
export function CompressionSection({ row, grade, nbRows, cs, showDetails, selLcomp, onSelLcomp }: {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        <div style={{ width: 3, height: 14, borderRadius: 2, background: ACCENT }} />
        <span style={{ fontSize: 12, fontWeight: 700, color: '#1e293b' }}>{t('bbhs_comp_title')} <Tex>{'N_{b,Rd}'}</Tex> (kN)</span>
        <span style={{ fontSize: 10, color: '#94a3b8', marginLeft: 4 }}>{t('bbhs_comp_ref')}</span>
      </div>
      ...
      <div className="bb-table-wrap" style={{ overflowX: 'auto', borderRadius: 8, border: '1px solid #e2e8f0', marginBottom: 10 }}>
        <table style={{ borderCollapse: 'collapse', fontSize: 11 }}>
          <thead>
            <tr>
              <th style={{ ...TH, textAlign: 'left', minWidth: 80 }}>{t('bbhs_comp_axis_col')}</th>
          ...
          <tbody>
            {/* Row 0 = Nb,y,Rd; Row 1 = Nb,z,Rd (same for SHS — still show both) */}
            {[0, 1].map(axis => (
              <tr key={axis} style={{ background: axis % 2 === 0 ? '#fff' : '#f8fafc' }}>
      ...
      <div style={{ fontSize: 10, color: '#64748b', marginBottom: 8, display: 'flex', gap: '0 12px', alignItems: 'baseline', flexWrap: 'wrap' }}>
        <span>{t('bbhs_comp_footer')}</span>
        <span><Tex>{'N_{pl,Rd}'}</Tex> = {r0(cs.Npl_Rd)} kN</span>
        <span style={{ opacity: 0.7 }}>{t('bbhs_comp_curve')}</span>
      </div>
...
export default function HollowCapacityPanel({ row: propRow, toolAccess = DEFAULT_ACCESS }: { row: SectionRow | null; toolAccess?: ToolAccess }) {
  const { t } = useTranslation()
    ...
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 12, color: '#94a3b8' }}>{t('bbhs_search_hint')}</span>
        </div>
  ...
  return (
    <>
      <CapacityShell
        ...
        gradeLabel={t('bbhs_grade_label')}
        tab={tab}
        onTab={setTab}
        tabLabels={{ results: t('bbhs_tab_results'), details: t('bbhs_tab_details') }}
        exportLabel={toolAccess.canExport ? `🖨 ${t('bbhs_exp_btn')}` : undefined}
      ...
      >
        {cs.isClass4 && (
          <div style={{ background: '#fef9c3', border: '1px solid #fde047', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#854d0e', marginBottom: 16 }}>
            {t('bbhs_header_class')} 4 — {t('bbhs_header_class4_note')}
          </div>
        )}
        ...
        {!cs.isClass4 && tab === 'results' && (
          <>
            <div style={{ marginBottom: 24 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <div style={{ width: 3, height: 14, borderRadius: 2, background: ACCENT }} />
                <span style={{ fontSize: 12, fontWeight: 700, color: '#1e293b', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{t('bbhs_cs_title')}</span>
                <span style={{ fontSize: 10, color: '#94a3b8', marginLeft: 4 }}>{t('bbhs_cs_ref')}</span>
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px 32px', padding: '12px 16px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 8, fontSize: 12, alignItems: 'baseline' }}>
                ...
                <span>{t('bbhs_header_class')} <strong>{cs.sectionClass}</strong></span>
              </div>
            </div>
            {cs.isCHS ? (
