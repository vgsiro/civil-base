...
import { calcPeakPressure, getMonoCpe } from '../wind-helpers'
...
export default function WindMono({ vb, setVb, cat, setCat, c0, setC0, rho, setRho, mD, setMD, mB, setMB, mH, setMH, alpha, setAlpha, cpiMin, setCpiMin, cpiMax, setCpiMax }: Props) {
  ...
  return (
    ...
    <div style={WIND_SECTION}>
      ...
      <TheoryBlock>
        <p><strong>EN 1991-1-4 §7.2.4</strong> — {t('std_ec1w_thy_wm_p1').split('—').slice(1).join('—').trim()}</p>
        <p><strong>{t('std_ec1w_thy_wm_theta0').split(':')[0].trim()}</strong>: {t('std_ec1w_thy_wm_theta0').split(':').slice(1).join(':').trim()}</p>
        <p><strong>{t('std_ec1w_thy_wm_zones').split(':')[0].trim()}</strong>: {t('std_ec1w_thy_wm_zones').split(':').slice(1).join(':').trim()}</p>
        <ul style={{ margin: '4px 0 4px 16px', padding: 0 }}>
          <li><strong>F</strong>: {t('std_ec1w_thy_wm_f').split(':').slice(1).join(':').trim()}</li>
          <li><strong>G</strong>: {t('std_ec1w_thy_wm_g').split(':').slice(1).join(':').trim()}</li>
          <li><strong>H</strong>: {t('std_ec1w_thy_wm_h').split(':').slice(1).join(':').trim()}</li>
          <li><strong>I</strong>: {t('std_ec1w_thy_wm_i').split(':').slice(1).join(':').trim()}</li>
        </ul>
        <p>{t('std_ec1w_thy_wm_interp')}</p>
      </TheoryBlock>
      <div style={FORMULA_BOX}>
        ...
        {theta === '180' && <>Table 7.3a, θ=180° (wind from high eave).</>}
      ...
      <div style={INPUT_GRID}>
        <div>
          <LabelTip tip={t('std_ec1w_tip_wm_d')}>d — {t('std_ec1w_det_rm_param_d')} (m)</LabelTip>
          <NumInput style={INPUT_STYLE} min={1} step={0.5} value={mD} onChange={setMD} />
        </div>
        <div>
          <LabelTip tip={t('std_ec1w_tip_wm_b')}>b — {t('std_ec1w_det_rm_param_b')} (m)</LabelTip>
          <NumInput style={INPUT_STYLE} min={1} step={0.5} value={mB} onChange={setMB} />
        </div>
        <div>
          <LabelTip tip={t('std_ec1w_tip_wm_h')}>h — {t('std_ec1w_det_rm_param_h_mono')} (m)</LabelTip>
          <NumInput style={INPUT_STYLE} min={1} step={0.5} value={mH} onChange={setMH} />
        </div>
        <div>
          <LabelTip tip={t('std_ec1w_tip_wm_alpha')}>α — {t('std_ec1w_det_rm_param_alpha')} (°)</LabelTip>
          <NumInput style={INPUT_STYLE} min={5} max={75} step={1} value={alpha} onChange={setAlpha} />
        </div>
        <div>
          <LabelTip tip={t('std_ec1w_tip_wm_theta')}>{t('std_ec1w_det_rm_param_theta')}</LabelTip>
        ...
        <div>
          <LabelTip tip={t('std_ec1w_tip_wm_cpe')}>{t('std_ec1w_det_ww_param_loaded')}</LabelTip>
      ...
      <div style={INPUT_GRID}>
        <div>
          <LabelTip tip={t('std_ec1w_tip_wm_cpi_min')}>c<sub>pi,min</sub></LabelTip>
          <NumInput style={INPUT_STYLE} min={-0.5} max={0} step={0.05} value={cpiMin} onChange={setCpiMin} />
        </div>
        <div>
          <LabelTip tip={t('std_ec1w_tip_wm_cpi_max')}>c<sub>pi,max</sub></LabelTip>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <DetailsSection id="mono-details">
          {(() => {
            ...
            const tableLabel = theta === '90' ? 'Table 7.3b (θ=90°)' : theta === '180' ? 'Table 7.3a (θ=180°)' : 'Table 7.3a (θ=0°)'
            return (<>
              ...
              <DetailGroup title={t('std_ec1w_det_input_data')}>
                <InputDataTable>
                  ...
                  <InputDataRow param={t('std_ec1w_det_rm_param_h_mono')} symbol={<Tex>{'h'}</Tex>} value={mH.toFixed(2)} unit="m" />
              ...
              <DetailGroup title={t('std_ec1w_det_ref_height')}>
                ...
                <CalcStep
                  label={<>{t('std_ec1w_det_lbl_ref_height')} <Tex>{'z_e'}</Tex> (EN 1991-1-4 §7.2.4)</>}
              ...
              <DetailGroup title={t('std_ec1w_det_press_coeff_type')}>
                <p style={P}>
                  EN 1991-1-4 §7.2.1(1) defines two types of pressure coefficients. The external pressure coefficient c<sub>pe,10</sub> applies to loaded areas greater than or equal to 10 m² and is used for overall structural design. The external pressure coefficient c<sub>pe,1</sub> applies to loaded areas of 1 m² or less and is used for the design of small elements. For intermediate areas between 1 m² and 10 m², a logarithmic interpolation is used. In these calculations, the <strong>{loadedArea === 'cpe10' ? <>c<sub>pe,10</sub> (loaded area ≥ 10 m²)</> : <>c<sub>pe,1</sub> (loaded area ≤ 1 m²)</>}</strong> values from {tableLabel} are used.
                </p>
              </DetailGroup>
              ...
              <DetailGroup title={t('std_ec1w_det_cpe_zones')}>
                ...
                <p style={P}>
                  {theta === '90'
                    ? `For θ = 90° (wind parallel to ridge), cpe values are taken from EN 1991-1-4 Table 7.3b and are independent of pitch angle α.`
                    : `For θ = ${theta}° (wind ${theta === '0' ? 'from the low eave' : 'from the high eave'}), cpe values are taken from EN 1991-1-4 Table 7.3a at pitch angle α = ${alpha}°. Values are linearly interpolated between tabulated angles where necessary.`
                  }
                </p>
                {['F', 'G', 'H', 'I'].map(zone => {
                  const { neg, pos } = cpe[zone] ?? { neg: 0, pos: null }
              ...
              <DetailGroup title={t('std_ec1w_det_cpi_group')}>
                <p style={P}>
                  The internal pressure coefficient c<sub>pi</sub> depends on the size and distribution of openings in the building envelope, as specified in EN 1991-1-4 §7.2.9. In these calculations:
                </p>
                <CalcStep
...
\\end{aligned}`}
              ...
              {theta === '0' && (
                <DetailGroup title="">
                  <p style={{ ...P, marginBottom: 6 }}>
                    For θ = 0° (wind from the low eave) both positive and negative c<sub>pe</sub> values may occur simultaneously. Two separate load cases must be checked:
                  </p>
                  ...
                  <p style={P}><Sub>{t('std_ec1w_det_rm_theta0_no_mix')}</Sub></p>
                </DetailGroup>
              )}
              ...
              <DetailGroup title={t('std_ec1w_det_add_notes')}>
                <ul style={{ fontSize: 12, color: '#374151', lineHeight: 1.75, margin: 0, paddingLeft: 18 }}>
                  ...
                  <li>{t('std_ec1w_det_rm_note_canopy_ref_pre')} {t('std_ec1w_det_rm_note_mono_canopy')}</li>
