...
function ConcreteGradeField({ fck, concreteName, onFck, onName, t }: {
  fck: number; concreteName: string
  onFck: (v: number) => void; onName: (v: string) => void; t: TFn
}) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div>
        <span style={{ ...LABEL_STYLE, marginBottom: 3, display: 'block' }}>{t('std_ec2rc_concrete_grade')}</span>
        <select value={selectVal} onChange={handleSelect}
          ...
          {isCustom && (
            <option value="custom">{t('std_ec2rc_custom_fck')} {fck} MPa</option>
          )}
        </select>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        <label style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <span style={{ ...LABEL_STYLE, marginBottom: 0 }}>
            {t('std_ec2rc_grade_name')}
            {isCustom && <span style={{ color: '#f59e0b', marginLeft: 4, fontSize: 9, fontWeight: 700 }}>{t('std_ec2rc_custom')}</span>}
          </span>
...
// ── Rebar row field ───────────────────────────────────────────────────────────
function ReinfField({ label, nbars, phi, onNbars, onPhi, onAdd, onRemove, t }: {
  label: string; nbars: number; phi: number
  onNbars: (v: number) => void; onPhi: (v: number) => void
  onAdd: () => void; onRemove: () => void; t: TFn
}) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        <Field label={t('std_ec2rc_no_bars')} value={nbars} onChange={v => onNbars(Math.max(0, Math.round(v)))} min={0} max={30} step={1}
          tip={t('std_ec2rc_tip_nbars_side')} />
        <Field label={t('std_ec2rc_dia')} value={phi} onChange={onPhi} unit="mm" min={8} max={40} step={2}
          tip={t('std_ec2rc_tip_side_dia')} />
      </div>
    </div>
  )
}
...
export function RectInputPanel({
  ...
  onReset, onResetUls, onResetSls, onResetSt,
  ulsResetKey, slsResetKey, stResetKey, rebarResetKey,
}: RectInputPanelProps) {
  ...
  return (
    <div className="rect-input-panel" style={{ width: 300, flexShrink: 0, minHeight: '100vh', overflowY: 'auto', overflowX: 'hidden', background: '#fff', borderRight: '1px solid #e2e8f0', padding: '12px 12px', display: 'flex', flexDirection: 'column', gap: 8, boxSizing: 'border-box' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
        ...
        <button
          onClick={showUls ? onResetUls : showSls ? onResetSls : onResetSt}
          style={{ fontSize: 10, color: '#64748b', background: 'none', border: '1px solid #e2e8f0', borderRadius: 4, padding: '2px 7px', cursor: 'pointer', whiteSpace: 'nowrap', lineHeight: 1.4 }}
        >
          {showUls ? t('std_ec2rc_reset_uls') : showSls ? t('std_ec2rc_reset_sls') : t('std_ec2rc_reset_st')}
        </button>
        <button onClick={onReset} style={{ fontSize: 10, color: '#64748b', background: 'none', border: '1px solid #e2e8f0', borderRadius: 4, padding: '2px 7px', cursor: 'pointer', whiteSpace: 'nowrap', lineHeight: 1.4 }}>
          {t('std_ec2rc_reset_all')}
        </button>
      </div>

      <Box key={`geometry-${ulsResetKey}`} title={t('std_ec2rc_geometry')}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <Field label={t('std_ec2rc_height')} value={Math.round(inp.h * 1000)} onChange={v => set('h', v / 1000)} unit="mm" min={100} max={3000} step={50}
            tip={t('std_ec2rc_tip_h')} />
          <Field label={t('std_ec2rc_width')}  value={Math.round(inp.b * 1000)} onChange={v => set('b', v / 1000)} unit="mm" min={100} max={2000} step={50}
            tip={t('std_ec2rc_tip_b')} />
        </div>
      </Box>

      <Box key={`materials-${ulsResetKey}`} title={t('std_ec2rc_materials')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <ConcreteGradeField
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ fontSize: 10, color: '#64748b', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 5, padding: '4px 8px', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' as const }}>
              ...
              {inp.ecmOverride != null && (
                <span style={{ color: '#f59e0b', fontWeight: 700 }}>→ {t('std_ec2rc_using')} {Math.round(inp.ecmOverride).toLocaleString()} MPa</span>
              )}
            </div>
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: '#1e293b' }}>{t('std_ec2rc_override')} E<sub>cm</sub></span>
            </label>
            {inp.ecmOverride != null && (
          ...
          <Field labelNode={<>f<sub>yk</sub></>} value={inp.fyk} onChange={v => set('fyk', v)} unit="MPa" min={400} max={600} step={50}
            tip={t('std_ec2rc_tip_fyk')} />
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
            <Field labelNode={<>α<sub>cc</sub></>} value={inp.acc} onChange={v => set('acc', v)} min={0.8} max={1} step={0.05}
              tip={t('std_ec2rc_tip_acc')} />
            <Field labelNode={<>γ<sub>c</sub></>}  value={inp.gc}  onChange={v => set('gc', v)}  min={1} max={2}   step={0.05}
              tip={t('std_ec2rc_tip_gc')} />
            <Field labelNode={<>γ<sub>s</sub></>}  value={inp.gs}  onChange={v => set('gs', v)}  min={1} max={1.5} step={0.05}
              tip={t('std_ec2rc_tip_gs')} />
          </div>
        </div>
      </Box>

      {showUls && <Box key={`uls-loads-${ulsResetKey}`} title={t('std_ec2rc_uls_loads')}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <Field labelNode={<>M<sub>Ed</sub> (+bot)</>}  value={inp.MEd} onChange={v => set('MEd', v)} unit="kNm" step={10}
            tip={t('std_ec2rc_tip_med')} />
          <Field labelNode={<>N<sub>Ed</sub> (−comp)</>} value={inp.NEd} onChange={v => set('NEd', v)} unit="kN"  step={50}
            tip={t('std_ec2rc_tip_ned')} />
        </div>
      </Box>}

      {showShear && <Box key={`st-loads-${stResetKey}`} title={t('std_ec2rc_shear_torsion_loads')}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <Field labelNode={<>V<sub>Ed</sub></>} value={inp.VEd ?? 0} onChange={v => set('VEd', v)} unit="kN" step={10}
            tip={t('std_ec2rc_tip_ved')} />
          <Field labelNode={<>T<sub>Ed</sub></>} value={inp.TEd ?? 0} onChange={v => set('TEd', v)} unit="kNm" step={5} min={0}
            tip={t('std_ec2rc_tip_ted')} />
          <Field labelNode={<>{t('std_ec2rc_stirrup_phi')}</>} value={inp.stirrup_phi ?? 10} onChange={v => set('stirrup_phi', v)} unit="mm" min={6} max={20} step={2}
            tip={t('std_ec2rc_tip_stirrup_phi')} />
          <Field labelNode={<>{t('std_ec2rc_spacing_s')}</>} value={inp.stirrup_s ?? 200} onChange={v => set('stirrup_s', v)} unit="mm" min={50} max={600} step={25}
            tip={t('std_ec2rc_tip_stirrup_s')} />
          {(() => {
            ...
            return (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <Field
                  labelNode={<>{t('std_ec2rc_legs')} n<sub>w</sub><span style={{ color: '#94a3b8', fontWeight: 400 }}> ({t('std_ec2rc_max')} {maxLegs})</span></>}
                  value={legs}
                  onChange={v => set('stirrup_legs', Math.min(Math.max(2, Math.round(v / 2) * 2), maxLegs))}
                  min={2} max={maxLegs} step={2}
                  tip={t('std_ec2rc_tip_legs_inp')}
                />
                {overLegs && (
                  <div style={{ fontSize: 10, color: '#dc2626', fontWeight: 600, lineHeight: 1.3 }}>
                    ⚠ {nTop} {t('std_ec2rc_only_top_bars_legs')} {maxLegs} {t('std_ec2rc_legs_suffix')}
                  </div>
                )}
              </div>
            )
        ...
        <div style={{ marginTop: 6, padding: '6px 8px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 6 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: '#7c3aed', fontWeight: 600, cursor: 'pointer', marginLeft: 'auto' }}>
              ...
              />
              {t('std_ec2rc_override')}
            </label>
          </div>
          {inp.theta_override && (
            <div style={{ marginTop: 4 }}>
              <Field
                labelNode={<>{t('std_ec2rc_theta_override')}</>}
        ...
        <div style={{ marginTop: 8, fontSize: 10, fontWeight: 700, color: '#334155', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('std_ec2rc_horizontal_ties')}</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 4 }}>
          <Field labelNode={<>{t('std_ec2rc_tie_phi')}</>} value={inp.tie_phi ?? 0} onChange={v => set('tie_phi', v)} unit="mm" min={0} max={20} step={2}
            tip={t('std_ec2rc_tip_tie_phi')} />
          {(() => {
            ...
            return (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <Field
                  labelNode={<>{t('std_ec2rc_no_ties')}{maxTies > 0 ? <span style={{ color: '#94a3b8', fontWeight: 400 }}> ({t('std_ec2rc_max')} {maxTies})</span> : null}</>}
                  value={tieN}
                  onChange={v => set('tie_n', Math.min(Math.max(1, Math.round(v)), maxTies > 0 ? maxTies : 99))}
                  min={1} max={maxTies > 0 ? maxTies : 20} step={1}
                  tip={t('std_ec2rc_tip_tie_n')}
                />
                {overLimit && (
                  <div style={{ fontSize: 10, color: '#dc2626', fontWeight: 600, lineHeight: 1.3 }}>
                    ⚠ {maxTies} {t('std_ec2rc_only_side_bars_avail')}
                  </div>
                )}
              </div>
            )
        ...
        {inp.nbars3 === 0 && (inp.tie_phi ?? 0) > 0 && (
          <div style={{ fontSize: 10, color: '#dc2626', fontWeight: 600, marginTop: 2 }}>
            ⚠ {t('std_ec2rc_no_side_bars_ties')}
          </div>
        )}
        <div style={{ fontSize: 10, color: '#64748b', fontStyle: 'italic', marginTop: 2 }}>{t('std_ec2rc_disable_tie_hint')}</div>
      </Box>}

      {showSls && <Box key={`sls-loads-${slsResetKey}`} title={t('std_ec2rc_sls_loads')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ fontSize: 10, color: '#64748b', fontStyle: 'italic' }}>
            {t('std_ec2rc_quasi_perm_hint')}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <Field labelNode={<>M<sub>sk</sub> (+bot)</>}  value={sls.Msk} onChange={v => setSls({ Msk: v })} unit="kNm" step={10}
              tip={t('std_ec2rc_tip_msk')} />
            <Field labelNode={<>N<sub>sk</sub> (−comp)</>} value={sls.Nsk} onChange={v => setSls({ Nsk: v })} unit="kN"  step={50}
              tip={t('std_ec2rc_tip_nsk')} />
          </div>
          <div>
            <span style={{ ...LABEL_STYLE, marginBottom: 3, display: 'block' }}>{t('std_ec2rc_crack_width_limit')}</span>
            <select
              value={sls.wk_lim}
              onChange={e => setSls({ wk_lim: parseFloat(e.target.value) })}
              style={{ ...SELECT_STYLE, padding: '5px 8px', fontSize: 12, width: '100%' }}
            >
              <option value={0.2}>{t('std_ec2rc_wk_02_aggressive')}</option>
              <option value={0.3}>{t('std_ec2rc_wk_03_typical')}</option>
              <option value={0.4}>{t('std_ec2rc_wk_04_mild')}</option>
            </select>
          </div>
          <div>
            <span style={{ ...LABEL_STYLE, marginBottom: 3, display: 'block' }}>k<sub>c</sub> — {t('std_ec2rc_kc_factor_desc')}</span>
            <select
              value={sls.kc}
              onChange={e => setSls({ kc: parseFloat(e.target.value) })}
              style={{ ...SELECT_STYLE, padding: '5px 8px', fontSize: 12, width: '100%' }}
            >
              <option value={0.4}>{t('std_ec2rc_kc_04_bending')}</option>
              <option value={0.9}>{t('std_ec2rc_kc_09_near_tension')}</option>
              <option value={1.0}>{t('std_ec2rc_kc_10_tension')}</option>
            </select>
          </div>
        </div>
      </Box>}
      ...
      {<Box key={`layer2-${rebarResetKey}`} title={t('std_ec2rc_layer2_top')}>
        <Field label={`${t('std_ec2rc_cover')} c₂`} value={Math.round(inp.c2 * 1000)} onChange={v => set('c2', v / 1000)} unit="mm" min={20} max={200} step={5}
          tip={t('std_ec2rc_tip_c2')} />
        <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
          {inp.rows2.map((row, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 6, alignItems: 'end' }}>
              <Field label={i === 0 ? t('std_ec2rc_bars_per_row') : ''} value={row.n} onChange={v => setRow2(i, { n: Math.max(1, Math.round(v)) })} min={1} max={30} step={1}
                tip={t('std_ec2rc_tip_nbars_top')} />
              <Field label={i === 0 ? t('std_ec2rc_dia_mm') : ''} value={row.phi} onChange={v => setRow2(i, { phi: v })} min={8} max={40} step={2}
                tip={t('std_ec2rc_tip_phi_top')} />
              {inp.rows2.length > 1
        ...
        <button onClick={addRow2} style={{ marginTop: 6, width: '100%', padding: '4px 0', borderRadius: 5, border: '1px solid #d1fae5', background: '#ecfdf5', color: '#047857', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>{t('std_ec2rc_add_row')}</button>
      </Box>}

      {<Box key={`layer1-${rebarResetKey}`} title={t('std_ec2rc_layer1_bottom')}>
        <Field label={`${t('std_ec2rc_cover')} c₁`} value={Math.round(inp.c1 * 1000)} onChange={v => set('c1', v / 1000)} unit="mm" min={20} max={200} step={5}
          tip={t('std_ec2rc_tip_c1')} />
        <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
          {inp.rows1.map((row, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 6, alignItems: 'end' }}>
              <Field label={i === 0 ? t('std_ec2rc_bars_per_row') : ''} value={row.n} onChange={v => setRow1(i, { n: Math.max(1, Math.round(v)) })} min={1} max={30} step={1}
                tip={t('std_ec2rc_tip_nbars_bot')} />
              <Field label={i === 0 ? t('std_ec2rc_dia_mm') : ''} value={row.phi} onChange={v => setRow1(i, { phi: v })} min={8} max={40} step={2}
                tip={t('std_ec2rc_tip_phi_bot')} />
              {inp.rows1.length > 1
        ...
        <button onClick={addRow1} style={{ marginTop: 6, width: '100%', padding: '4px 0', borderRadius: 5, border: '1px solid #d1fae5', background: '#ecfdf5', color: '#047857', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>{t('std_ec2rc_add_row')}</button>
      </Box>}

      {<Box key={`layer3-${rebarResetKey}`} title={t('std_ec2rc_layer3_sides')}>
        <label style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: hasSideBars ? 8 : 0, cursor: 'pointer' }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600, color: '#1e293b' }}>{t('std_ec2rc_include_side_bars')}</span>
        </label>
        {hasSideBars && (
          <ReinfField label={t('std_ec2rc_side_bars_per_side')}
            nbars={inp.nbars3} phi={inp.phi3}
            onNbars={v => set('nbars3', v)} onPhi={v => set('phi3', v)}
            onAdd={addSide} onRemove={removeSide} t={t} />
        )}
      </Box>}

      {/* ── First-order / Second-order effects — ULS only ────────────────────── */}
      {showUls && <Box key={`uls-first-${ulsResetKey}`} title={t('std_ec2rc_first_order')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <CbRow checked={!!inp.firstOrder} onChange={v => set('firstOrder', v)}>
            <strong>{t('std_ec2rc_enable_first_order')}</strong>
          </CbRow>
          {inp.firstOrder && <>
      ...
      {showUls && <Box key={`uls-second-${ulsResetKey}`} title={t('std_ec2rc_second_order')}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <CbRow checked={!!inp.secondOrder} onChange={v => set('secondOrder', v)}>
            <strong>{t('std_ec2rc_nominal_curvature')}</strong> (EC2 §5.8.8)
          </CbRow>
          {inp.secondOrder && <>
            <Field label={t('std_ec2rc_effective_length')} value={inp.l0 ?? l0Default} onChange={v => set('l0', v)} unit="m" min={0.1} step={0.5}
              tip={t('std_ec2rc_tip_l0')} />
            {!inp.firstOrder && (
              <CbRow checked={!!inp.useImperfection} onChange={v => set('useImperfection', v)} indent>
                {t('std_ec2rc_geom_imperfection')} e<sub>a</sub> = l₀ / 400
              </CbRow>
            ...
            <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: 6, display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                {t('std_ec2rc_effective_creep')} φ<sub>ef</sub>
              </div>
              <Field
                labelNode={<>φ(∞,t₀) — {t('std_ec2rc_long_term_creep')}</>}
                value={inp.phi_inf ?? 1.5}
                onChange={v => set('phi_inf', v)}
                min={0} max={5} step={0.1}
                tip={t('std_ec2rc_tip_phi_creep')}
              />
              <Field
                labelNode={<>M<sub>0Eqp</sub> — {t('std_ec2rc_quasi_perm_sls_moment')}</>}
                value={inp.M0Eqp ?? Math.round(inp.MEd * 0.6)}
                onChange={v => set('M0Eqp', v)}
                unit="kNm" step={10}
                tip={t('std_ec2rc_tip_m0eqp')}
              />
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: '#1e293b' }}>
                  M<sub>0Ed</sub> — {t('std_ec2rc_uls_moment_ref')}
                </span>
                <select
                  value={inp.M0EdRef ?? '1'}
                  onChange={e => set('M0EdRef', e.target.value as '1' | '2')}
                  style={{ ...SELECT_STYLE, padding: '5px 8px', fontSize: 12 }}>
                  <option value="1">M₁ ({t('std_ec2rc_end1_bottom').toLowerCase()})</option>
                  <option value="2">M₂ ({t('std_ec2rc_end2_top').toLowerCase()})</option>
                </select>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 5, padding: '5px 8px', fontSize: 11 }}>
                ...
                <span style={{ fontFamily: 'ui-monospace,monospace', fontWeight: 700, color: inp.phi_ef != null ? '#94a3b8' : '#0369a1' }}>
                  ...
                  {inp.phi_ef != null && <span style={{ color: '#f59e0b', marginLeft: 4 }}>({t('std_ec2rc_overridden')})</span>}
                </span>
              </div>
              <CbRow checked={inp.phi_ef != null} onChange={v => set('phi_ef', v ? res.phi_ef_calc : undefined)} indent>
                {t('std_ec2rc_override_phi_ef')}
              </CbRow>
              {inp.phi_ef != null && (
                <div style={{ paddingLeft: 16 }}>
                  <Field
                    labelNode={<>φ<sub>ef</sub> — {t('std_ec2rc_phi_ef_override_val')}</>}
                    value={inp.phi_ef}
                    onChange={v => set('phi_ef', v)}
                    min={0} max={10} step={0.1}
                    tip={t('std_ec2rc_tip_phi_ef_inp')}
                  />
                </div>
              )}
            </div>
