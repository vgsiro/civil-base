...
import { Tooltip } from '@/app/(home)/standards/_lib/ui'
...
export function Field({ label, labelNode, value, onChange, unit, min, max, step, tip }: {
  ...
  const commit = (raw: string) => {
    ...
    if (!isFinite(v)) { setStr(String(value)); return }
  ...
  const stepBy = (dir: 1 | -1) => {
    ...
    const s = step ?? 1
    const next = isFinite(cur) ? cur + dir * s : value + dir * s
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      <span style={{ ...LABEL_STYLE, marginBottom: 0, display: 'flex', alignItems: 'center', minHeight: 28 }}>
...
export function Box({ title, children, accent, titleRight }: { title: string; children: React.ReactNode; accent?: string; titleRight?: React.ReactNode }) {
  return (
    <div style={{ border: '1px solid #e2e8f0', borderRadius: 8 }}>
      <div style={{ padding: '6px 12px', background: accent ? `${accent}14` : '#f8fafc', borderBottom: '1px solid #e2e8f0', fontSize: 11, fontWeight: 700, color: accent ?? '#1e293b', letterSpacing: '0.04em', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span>{title}</span>
        {titleRight}
      </div>
