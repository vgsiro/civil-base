...
const EMPTY_TABLES: Record<string, RawTable[]> = { 'en1990': [], 'en1990-a1': [], 'en1990-a2': [] }
...
function ResizablePanel({ defaultWidth, minWidth = PANEL_MIN, maxWidth = PANEL_MAX, accentHover = '#a5b4fc', children }: {
...
export default function Ec0TablesPanel({ section, pageKey, onNavChange, editMode = false, onEditDone }: {
  section: string; pageKey?: string; onNavChange: (key: string, val: string) => void
  editMode?: boolean; onEditDone?: () => void
}) {
  const { t } = useTranslation()
  const { dirty, saving, stage, save, discard } = useAdminEdit('ec0', editMode, onEditDone)

  const defaultParts = useMemo(() => [
    { id: 'en1990',    code: 'EN 1990',          label: t('std_tbl_ec0_basis') },
    { id: 'en1990-a1', code: 'EN 1990 Annex A1', label: t('std_tbl_ec0_a1')   },
    { id: 'en1990-a2', code: 'EN 1990 Annex A2', label: t('std_tbl_ec0_a2')   },
  ], [t])

  const DEFAULT_TABLES = useMemo<Record<string, RawTable[]>>(() => ({
    'en1990':    [{ id: 'psi_factors',     number: 'Table A1.1',    name: t('std_ec0_tbl_psi_name')     }],
    'en1990-a1': [{ id: 'partial_factors', number: 'Table A1.2(B)', name: t('std_ec0_tbl_partial_name') }],
    'en1990-a2': [],
  }), [t])
  ...
  useEffect(() => {
    getEcContent<typeof defaultParts>('ec0', 'tables_parts', 'parts').then(saved => { if (saved) { setDbParts(saved); setParts(saved) } })
    getEcContent<Record<string, RawTable[]>>('ec0', 'tables_parts', 'tables_by_part').then(saved => { if (saved) { setDbTables(saved); setTablesByPart(saved) } })
  }, [])
      ...
      <ResizablePanel defaultWidth={190} maxWidth={300} accentHover="#a5b4fc">
        {collapsed => collapsed
          ? (
            ...
            <>
              <div style={{ padding: '10px 12px 6px', fontSize: 10, fontWeight: 700, color: '#94a3b8', letterSpacing: '0.07em', textTransform: 'uppercase', borderBottom: '1px solid #f1f5f9', flexShrink: 0 }}>EN 1990 {t('std_tbl_parts_label')}</div>
              {parts.map(p => {
                const empty = (tablesByPart[p.id] ?? []).length === 0
                return (
