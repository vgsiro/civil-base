  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0' }}>
      <a href={`/u/${profile.username}`} style={{ textDecoration: 'none', flexShrink: 0 }}>
        <div style={{ width: 40, height: 40, borderRadius: '50%', background: AVATAR_COLORS[profile.avatar_color ?? 0], display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 800, color: '#fff', overflow: 'hidden' }}>
          {profile.avatar_url ? <img src={profile.avatar_url} alt={initial} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : initial}
        </div>
      </a>
