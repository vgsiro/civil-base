...
import { Settings, LogOut, UserCircle, ChevronRight, ChevronLeft, Languages, KeyRound, Eye, EyeOff, ArrowUpCircle, BadgeCheck } from 'lucide-react'
import { supabase } from '@/lib/supabase'
import type { User } from '@supabase/supabase-js'
import { useTranslation } from '../../../i18n/LanguageContext'
import { type Locale, LANGUAGE_OPTIONS } from '../../../i18n/index'
import { useSubscription, tierLabel, tierColor, tierBg } from '@/lib/useSubscription'
import { UpgradeRequestModal } from '../../shared/UpgradeRequestModal'
...
export default function AccountMenu({ user, profile, avatarColor = 0, avatarUrl, displayName, profileUsername, size = 32, dark = false, onSignOut }: Props) {
  ...
  const tier = useSubscription(user.id, user.email)
  ...
  useEffect(() => {
    supabase
      ...
      .limit(1)
      .maybeSingle()
      .then(({ data }) => setSubExpiresAt(data?.expires_at ?? null))
  }, [user.id])
  ...
  const canUpgrade = tier === 'normal' || tier === 'pro'
  const expired = subExpiresAt && new Date(subExpiresAt) < new Date()
  const expiresLabel = subExpiresAt && !expired
    ? `Expires ${new Date(subExpiresAt).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' })}`
    : expired ? 'Expired' : tier === 'main_admin' || tier === 'admin' ? '∞ Permanent' : null

  return (
    <div ref={ref} style={{ position: 'relative', flexShrink: 0 }}>
      {showUpgradeModal && (
        <UpgradeRequestModal
          defaultTier={tier === 'pro' ? 'premium' : 'pro'}
          onClose={() => setShowUpgradeModal(false)}
        />
      )}
      {showVerifyModal && profile && (
        <VerifyModal
          user={user}
          profile={profile}
        ...
        <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, background: '#fff', borderRadius: 12, boxShadow: '0 8px 40px rgba(0,0,0,0.18)', border: '1px solid #e4e6eb', zIndex: 400, width: 290, overflow: 'hidden' }}>
          ...
          {panel === 'main' && (
            <>
              ...
              {/* Professional status — always shown */}
              {profile && (
                <>
                  <div style={{ padding: '4px 8px 2px' }}>
                    {profile.is_verified ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 10, border: '1px solid #10b98130', background: 'linear-gradient(135deg, #f0fdf4, #ecfdf5)' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 13, fontWeight: 700, color: '#065f46' }}>Verified Professional</div>
                          <div style={{ fontSize: 11, color: '#059669', marginTop: 1 }}>
                            {profile.profession ?? 'Civil Engineering Expert'}
                          </div>
                        </div>
                      ...
                      <button
                        ...
                        style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '9px 12px', borderRadius: 10, border: '1px solid #3b82f620', background: 'linear-gradient(135deg, #eff6ff, #f0fdf4)', cursor: 'pointer', textAlign: 'left' as const }}
                        onMouseEnter={e => { e.currentTarget.style.background = 'linear-gradient(135deg, #dbeafe, #dcfce7)' }}
                        onMouseLeave={e => { e.currentTarget.style.background = 'linear-gradient(135deg, #eff6ff, #f0fdf4)' }}>
                        <div style={{ width: 34, height: 34, borderRadius: '50%', background: profile.pending_profession ? 'linear-gradient(135deg, #f59e0b, #f97316)' : 'linear-gradient(135deg, #3b82f6, #10b981)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <BadgeCheck size={18} color="#fff" />
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 13, fontWeight: 700, color: profile.pending_profession ? '#92400e' : '#1e40af' }}>
                            {profile.pending_profession ? 'Verification pending…' : 'Get Professional'}
                          </div>
                          <div style={{ fontSize: 11, color: '#64748b', marginTop: 1 }}>
                            {profile.pending_profession ? 'Your application is under review' : 'Verify your expertise & unlock features'}
                          </div>
                        </div>
                        {!profile.pending_profession && <ChevronRight size={14} color="#3b82f6" style={{ flexShrink: 0 }} />}
                      </button>
                    )}
                  </div>
                  <div style={{ height: 1, background: '#e4e6eb', margin: '4px 0' }} />
              ...
              {/* Subscription card */}
              <div style={{ padding: '4px 8px 2px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', borderRadius: 10, background: tierBg(tier), border: `1px solid ${tierColor(tier)}30` }}>
                  <div>
                    <div style={{ fontSize: 11, color: '#64748b', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.06em', marginBottom: 3 }}>Subscription</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontSize: 12, fontWeight: 800, color: tierColor(tier), background: '#fff', padding: '1px 8px', borderRadius: 10, border: `1px solid ${tierColor(tier)}40` }}>
                        {tierLabel(tier)}
                      </span>
                      {expiresLabel && (
                        <span style={{ fontSize: 11, color: expired ? '#ef4444' : '#64748b', fontWeight: expired ? 700 : 400 }}>{expiresLabel}</span>
                      )}
                    </div>
                  </div>
                  {canUpgrade && (
                    <button
                      ...
                      style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 8, border: 'none', background: tierColor(tier === 'normal' ? 'pro' : 'premium'), color: '#fff', fontSize: 11, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' as const }}
                      onMouseEnter={e => { e.currentTarget.style.opacity = '0.85' }}
                      onMouseLeave={e => { e.currentTarget.style.opacity = '1' }}>
                      <ArrowUpCircle size={12} />
                      {tier === 'normal' ? 'Upgrade' : 'Go Premium'}
                    </button>
                  )}
                </div>
              </div>
              ...
              <div style={{ padding: '4px 8px' }}>
                <a href="/donate" onClick={close}
                  style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '8px 10px', borderRadius: 8, textDecoration: 'none' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = '#fffbeb' }}
                  onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background = 'none' }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#fef3c7', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 18 }}>
                    ☕
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: '#92400e' }}>Support CivilAxis</div>
                    <div style={{ fontSize: 12, color: '#b45309', marginTop: 1 }}>Buy me a coffee ☕</div>
                  </div>
                </a>
              </div>
