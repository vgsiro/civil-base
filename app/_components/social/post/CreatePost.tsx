...
export default function CreatePost({ user, avatarColor, avatarUrl, displayName, onPostCreated, resharePost, onCancelReshare }: CreatePostProps) {
  ...
  useEffect(() => {
    function onPaste(e: ClipboardEvent) {
      ...
      // ignore pastes originating from any input/textarea (e.g. chat box)
    ...
    <div
      ...
      <div style={{ padding: '16px 18px 10px', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                {TABS.map(tab => (
                  <button key={tab.type} onClick={() => setActiveType(tab.type)}
                    style={{ display: 'flex', alignItems: 'center', gap: 3, padding: '2px 7px', borderRadius: 20, border: `1px solid ${activeType === tab.type ? '#3b82f6' : '#e2e8f0'}`, background: activeType === tab.type ? '#eff6ff' : '#f8fafc', color: activeType === tab.type ? '#3b82f6' : '#64748b', fontSize: 10, fontWeight: activeType === tab.type ? 600 : 400, cursor: 'pointer' }}>
                    {tab.icon} {tab.label}
                  </button>
                ))}
              </div>
          ...
          />
          {!isReshare && (
            <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>Tip: drag & drop or Ctrl+V to add an image</div>
          )}
        </div>
      </div>
