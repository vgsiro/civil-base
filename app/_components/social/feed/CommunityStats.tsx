...
const METRICS = [
  { id: 'views',    labelKey: 'cs_total_access' as const, color: '#3b82f6', icon: Eye },
  { id: 'members',  labelKey: 'cs_total_users'  as const, color: '#10b981', icon: Users },
  { id: 'posts',    labelKey: 'cs_new_posts'    as const, color: '#f59e0b', icon: FileText },
  { id: 'verified', labelKey: 'cs_verified'     as const, color: '#8b5cf6', icon: BadgeCheck },
] as const
          ...
          {METRICS.map(m => (
            <button key={m.id} onClick={() => { onChange(m.id); setOpen(false) }} style={{
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                <m.icon size={13} color={m.color} /> {t(m.labelKey)}
              </span>
    ...
    <div style={{ background: '#1e293b', borderRadius: 12, border: '1px solid #334155', padding: '20px 24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap' as const, gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
          ...
          <span style={{ fontSize: 14, color: '#64748b', fontWeight: 600 }}>{t(activeMeta.labelKey)}</span>
        </div>
