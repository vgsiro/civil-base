...
import type { PostVisibility } from '../../_types'
...
const VISIBILITY_OPTIONS: { value: PostVisibility; labelKey: string; subKey: string; icon: React.ReactNode }[] = [
  { value: 'public',  labelKey: 'home_photo_visibility_public',  subKey: 'home_photo_visibility_public_sub',  icon: <Globe size={16} /> },
  { value: 'friends', labelKey: 'home_photo_visibility_friends', subKey: 'home_photo_visibility_friends_sub', icon: <Users size={16} /> },
  { value: 'private', labelKey: 'home_photo_visibility_private', subKey: 'home_photo_visibility_private_sub', icon: <Lock size={16} /> },
]
...
export default function PhotoModal({
  ...
  async function save() {
    ...
    if (uploadErr) { setSaving(false); setMsg(t('home_photo_upload_failed') + uploadErr.message); return }
    ...
    if (shareUpdate) {
      const defaultCaption = isAvatar ? t('home_photo_caption_avatar') : t('home_photo_caption_cover')
      ...
      if (postErr) { setSaving(false); setMsg(t('home_photo_post_failed') + postErr.message); return }
    }

    setSaving(false); setMsg(t('home_photo_saved'))
  ...
  const hint = isAvatar
    ? t('home_photo_drag_avatar_hint')
    : t('home_photo_drag_cover_hint')
    ...
    <>
      ...
      {confirmDiscard && (
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 801, background: '#fff', borderRadius: 14, boxShadow: '0 16px 60px rgba(0,0,0,0.28)', width: 340, padding: '24px 24px 20px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ fontSize: 17, fontWeight: 800 }}>{t('home_photo_discard_title')}</div>
            <div style={{ fontSize: 14, color: '#65676b', lineHeight: 1.5 }}>{t('home_photo_discard_body')}</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
              <button onClick={onClose} style={{ width: '100%', padding: '10px 0', borderRadius: 8, border: 'none', background: '#ef4444', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>{t('home_photo_discard')}</button>
              <button onClick={() => setConfirmDiscard(false)} style={{ width: '100%', padding: '10px 0', borderRadius: 8, border: 'none', background: '#f0f2f5', color: '#050505', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>{t('home_photo_continue_editing')}</button>
            </div>
          </div>
        </>
      )}
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 601, background: '#fff', borderRadius: 16, boxShadow: '0 24px 80px rgba(0,0,0,0.3)', width: '92vw', maxWidth: 620, display: 'flex', flexDirection: 'column', maxHeight: '92vh', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid #e4e6eb', flexShrink: 0 }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: '#050505' }}>{t('home_photo_edit_title')}</div>
        ...
        <div style={{ display: 'flex', gap: 4, padding: '8px 20px', borderBottom: '1px solid #e4e6eb', flexShrink: 0 }}>
          {(['avatar', 'cover'] as Tab[]).map(tb => (
            <button key={tb} onClick={() => setTab(tb)}
              style={{ padding: '6px 16px', borderRadius: 8, border: 'none', background: tab === tb ? '#e7f3ff' : 'none', color: tab === tb ? '#3b82f6' : '#65676b', fontSize: 14, fontWeight: tab === tb ? 700 : 500, cursor: 'pointer' }}>
              {tb === 'avatar' ? t('home_photo_tab_avatar') : t('home_photo_tab_cover')}
            </button>
          ))}
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {imgSrc ? (
              <div
                ...
                <div style={{
                  ...
                  {isAvatar ? t('home_photo_drag_reposition') : t('home_photo_drag_updown')}
                </div>
              </div>
            ) : (
              /* Placeholder */
          ...
          {imgSrc && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <button onClick={() => { setZoom(minZoom); setOffset({ x: 0, y: 0 }) }} title={t('home_photo_reset')}
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <button onClick={() => fileRef.current?.click()}
              ...
              <Upload size={16} /> {imgSrc ? t('home_photo_change') : t('home_photo_upload')}
            </button>
        ...
        {imgSrc && (
          <div style={{ padding: '12px 20px', borderTop: '1px solid #e4e6eb', display: 'flex', flexDirection: 'column', gap: 10, flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => setShareUpdate(v => !v)}>
              ...
              <span style={{ fontSize: 14, fontWeight: 600, color: '#050505', userSelect: 'none' as const }}>{t('home_photo_share_to_feed')}</span>
            </div>

            {shareUpdate && (
              <>
                <textarea value={postCaption} onChange={e => setPostCaption(e.target.value)}
                  placeholder={isAvatar ? t('home_photo_caption_avatar_ph') : t('home_photo_caption_cover_ph')}
                  rows={2}
                ...
                <div style={{ position: 'relative', alignSelf: 'flex-start' as const }}>
                  <button onClick={() => setShowVisibilityMenu(v => !v)}
                    ...
                    {visOption.icon}{t(visOption.labelKey as any)}<ChevronDown size={14} color="#65676b" />
                  </button>
                  {showVisibilityMenu && (
                    <>
                      ...
                      <div style={{ position: 'absolute', bottom: 'calc(100% + 6px)', left: 0, zIndex: 701, background: '#fff', borderRadius: 10, boxShadow: '0 -4px 30px rgba(0,0,0,0.16)', border: '1px solid #e4e6eb', overflow: 'hidden', minWidth: 220 }}>
                        {VISIBILITY_OPTIONS.map(opt => (
                          <button key={opt.value} onClick={() => { setVisibility(opt.value); setShowVisibilityMenu(false) }}
                            ...
                            <div>
                              <div style={{ fontSize: 13, fontWeight: 700, color: visibility === opt.value ? '#3b82f6' : '#050505' }}>{t(opt.labelKey as any)}</div>
                              <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 1 }}>{t(opt.subKey as any)}</div>
                            </div>
        ...
        <div style={{ padding: '12px 20px', borderTop: '1px solid #e4e6eb', display: 'flex', justifyContent: 'flex-end', gap: 8, flexShrink: 0 }}>
          <button onClick={tryClose} style={{ padding: '9px 18px', borderRadius: 8, border: 'none', background: '#e4e6eb', color: '#050505', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>{t('home_cancel')}</button>
          <button onClick={save} disabled={!imgSrc || saving}
            ...
            <Check size={16} /> {saving ? t('home_saving') : t('home_photo_save')}
          </button>
        </div>
      </div>
    </>
