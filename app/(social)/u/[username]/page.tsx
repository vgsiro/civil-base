...
import {
  ...
  UserCheck, UserX, Clock, X, MoreHorizontal, Ban, Flag, Bookmark,
} from 'lucide-react'
import { supabase } from '@/lib/supabase'
import type { User } from '@supabase/supabase-js'
import type { Profile, ProfileStats, PostWithProfile, FeaturedLink } from '../../../_types'
import PostCard from '../../../_components/social/post/PostCard'
...
import ReportModal from '../../../_components/shared/ReportModal'
import { createNotification, deleteNotification } from '../../../_lib/notify'
import { blockUser, isBlocked, unblockUser, isBlockedByThem } from '../../../_lib/moderation'
...
export default function PublicProfilePage() {
  ...
  async function attachResharedPosts(fetchedPosts: PostWithProfile[]): Promise<PostWithProfile[]> {
    const ids = [...new Set(fetchedPosts.filter(p => p.reshared_post_id).map(p => p.reshared_post_id!))]
    if (ids.length === 0) return fetchedPosts
    ...
    return fetchedPosts.map(p => p.reshared_post_id ? { ...p, reshared_post: map[p.reshared_post_id] ?? null } : p)
  }
  ...
  useEffect(() => { document.title = profile ? `${profile.display_name ?? profile.username} — CivilAxis` : `@${username} — CivilAxis` }, [profile, username])
  ...
  const [openChats, setOpenChats] = useState<{ convId: string; peer: any; startFocused?: boolean; showAfter?: string }[]>([])
  ...
  const removeConvCallbackRef = useRef<((convId: string) => void) | null>(null)
  const stableRegisterMarkRead = useCallback((fn: (convId: string) => void) => { markConvReadRef.current = fn }, [])
  const stableRegisterSent = useCallback((fn: (convId: string, body: string) => void) => { sentCallbackRef.current = fn }, [])
  const stableRegisterRemoveConv = useCallback((fn: (convId: string) => void) => { removeConvCallbackRef.current = fn }, [])
  ...
  // Profile (...) menu: block / report (other profile)
  const [moreMenuOpen, setMoreMenuOpen] = useState(false)
  const [showReport, setShowReport] = useState(false)
  const [confirmBlock, setConfirmBlock] = useState(false)
  const [blocked, setBlocked] = useState(false)
  const moreMenuRef = useRef<HTMLDivElement>(null)
  // Own profile (...) menu: recents / saved
  const [ownMoreMenuOpen, setOwnMoreMenuOpen] = useState(false)
  const ownMoreMenuRef = useRef<HTMLDivElement>(null)
  ...
  useEffect(() => {
    if (!username) return
    async function load() {
      // Seed the logged-in user from the cached session first (near-instant, from localStorage)
      // so the nav renders logged-in immediately — avoids flashing the signed-out nav on load.
      const { data: { session } } = await supabase.auth.getSession()
      if (session?.user) {
        setCurrentUser(session.user)
        // Seed the cached current-user profile too, so the nav avatar shows the user's real
        // avatar image right away instead of the initials/signup-default until cpData loads.
        try {
      ...
      // profile, profile_stats and getUser() are mutually independent (stats is keyed by the
      // username we already have, auth needs nothing), so fire all three together instead of
      // waiting on the profile row first. Only posts needs the resolved profile id.
      const [{ data: profileData }, { data: statsData }, { data: { user: authUser } }] = await Promise.all([
        supabase.from('profiles').select('*').eq('username', username).maybeSingle(),
        supabase.from('profile_stats').select('*').eq('username', username).maybeSingle(),
      ...
      // Posts are the remaining piece of the first paint — the only query that needs prof.id.
      const postsRes = await supabase.from('posts')
        .select('*, profiles!posts_user_id_fkey(id,username,family_name,given_name,display_name,full_name,profession,specializations,is_verified,avatar_color,avatar_url), post_likes(user_id), post_comments(id), post_recommendations(user_id)')
        .eq('user_id', prof.id)
        .order('created_at', { ascending: false })
        .limit(20)
      if (postsRes.error) console.error('posts fetch error:', postsRes.error.message)
      setPosts(await attachResharedPosts((postsRes.data as PostWithProfile[]) ?? []))
      setLoading(false)

      if (!authUser) return

      // Secondary, non-blocking: block check + viewer-specific state.
      if (authUser.id !== prof.id) {
        const theyBlockedMe = await isBlockedByThem(authUser.id, prof.id)
        if (theyBlockedMe) { setNotFound(true); return }
      }

      const { data: cpData } = await supabase
        .from('profiles').select('*').eq('id', authUser.id).maybeSingle()
      setCurrentProfile(cpData as Profile | null)
      try { if (cpData) localStorage.setItem(`civilbase_profile_${authUser.id}`, JSON.stringify(cpData)) } catch {}
  ...
  // Load whether the current user has blocked this profile
  useEffect(() => {
    if (!currentUser || !profile || currentUser.id === profile.id) return
    isBlocked(currentUser.id, profile.id).then(setBlocked)
  }, [currentUser, profile])
  ...
  useEffect(() => {
    if (!ownMoreMenuOpen) return
    function handler(e: MouseEvent) {
      if (ownMoreMenuRef.current && !ownMoreMenuRef.current.contains(e.target as Node)) setOwnMoreMenuOpen(false)
    }
    document.addEventListener('mousedown', handler)
    return () => document.removeEventListener('mousedown', handler)
  }, [ownMoreMenuOpen])
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#f0f2f5' }}>
        <TopNavBar
          ...
          onNewMessage={(convId, peer, showAfter) => {
            setOpenChats(prev => prev.some(c => c.convId === convId) ? prev : [...prev, { convId, peer, startFocused: false, showAfter }])
          }}
          onRegisterMarkRead={stableRegisterMarkRead}
          onRegisterSent={stableRegisterSent}
          onRegisterRemoveConv={stableRegisterRemoveConv}
          openConvIds={openChats.map(c => c.convId)}
          onOpenPost={postId => setOpenPostId(postId)}
        />
        {/* Skeleton: cover band + header placeholder, so the page keeps its shape while loading */}
        <div style={{ background: '#fff', borderBottom: '1px solid #e4e6eb' }}>
          <div style={{ maxWidth: 1050, margin: '0 auto' }}>
            <div style={{ height: 348, borderRadius: '0 0 8px 8px', background: '#e4e6eb' }} />
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 16, padding: '0 24px', marginTop: -40 }}>
              <div style={{ width: 168, height: 168, borderRadius: '50%', background: '#d8dadd', border: '4px solid #fff' }} />
              <div style={{ flex: 1, paddingBottom: 16 }}>
                <div style={{ width: 220, height: 22, borderRadius: 6, background: '#e4e6eb', marginBottom: 8 }} />
                <div style={{ width: 140, height: 14, borderRadius: 6, background: '#eceef0' }} />
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f2f5' }}>
      ...
      {showReport && currentUser && profile && (
        <ReportModal
          reporterId={currentUser.id}
          reportedId={profile.id}
          reportedName={profile.display_name || profile.full_name || profile.username || 'this user'}
      ...
      {confirmBlock && profile && (
        <div onClick={() => setConfirmBlock(false)}
          ...
          <div onClick={e => e.stopPropagation()}
            style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 380, padding: '22px 22px 18px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ fontSize: 17, fontWeight: 800, color: '#0f172a', marginBottom: 8 }}>
              Block {profile.display_name || profile.full_name || profile.username}?
            </div>
            <div style={{ fontSize: 13, color: '#475569', lineHeight: 1.6, marginBottom: 20 }}>
              They won't be able to message you, and you won't see their messages or posts. You can unblock them anytime from their profile.
            </div>
      ...
      {showMessageBox && convId && currentUser && profile && (
        <ChatBox
          ...
          onMarkRead={cid => markConvReadRef.current?.(cid)}
          onSent={(convId, body) => sentCallbackRef.current?.(convId, body)}
          index={0}
        />
      )}
      ...
      {currentUser && openChats.map((chat, i) => (
        <ChatBox key={chat.convId} convId={chat.convId} userId={currentUser.id} peer={chat.peer} index={i}
          ...
          onMarkRead={convId => markConvReadRef.current?.(convId)}
          onSent={(convId, body) => sentCallbackRef.current?.(convId, body)}
          onDeleteChat={convId => removeConvCallbackRef.current?.(convId)} />
      ))}
            ...
            <div style={{ flex: 1, paddingTop: 44, paddingBottom: 12, minWidth: 0 }}>
              ...
              {isOwnProfile ? (
                <>
                  ...
                  <div ref={ownMoreMenuRef} style={{ position: 'relative' }}>
                    <button onClick={() => setOwnMoreMenuOpen(v => !v)}
                      aria-label="More options"
                      style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36, borderRadius: 8, border: 'none', background: '#e4e6eb', color: '#050505', cursor: 'pointer' }}
                      onMouseEnter={e => { e.currentTarget.style.background = '#d8dadf' }}
                      onMouseLeave={e => { e.currentTarget.style.background = '#e4e6eb' }}>
                      <MoreHorizontal size={18} />
                    </button>
                    {ownMoreMenuOpen && (
                      <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: 6, minWidth: 190, background: '#fff', borderRadius: 10, boxShadow: '0 8px 28px rgba(0,0,0,0.18)', border: '1px solid #e4e6eb', zIndex: 50, overflow: 'hidden', padding: '4px 0' }}>
                        <a href="/recents"
                          style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px 14px', fontSize: 14, color: '#050505', background: 'none', textDecoration: 'none' }}
                          ...
                          <Clock size={16} color="#65676b" /> Recent activity
                        </a>
                        <a href="/saved"
                          style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px 14px', fontSize: 14, color: '#050505', background: 'none', textDecoration: 'none' }}
                          onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = '#f0f2f5' }}
                          onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background = 'none' }}>
                          <Bookmark size={16} color="#65676b" /> Saved posts
                        </a>
                      </div>
                    )}
                  </div>
                ...
                <>
                  ...
                  <div ref={moreMenuRef} style={{ position: 'relative' }}>
                    ...
                    {moreMenuOpen && (
                      <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: 6, minWidth: 180, background: '#fff', borderRadius: 10, boxShadow: '0 8px 28px rgba(0,0,0,0.18)', border: '1px solid #e4e6eb', zIndex: 50, overflow: 'hidden', padding: '4px 0' }}>
                        <button
                          onClick={() => { setMoreMenuOpen(false); setShowReport(true) }}
                          style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px 14px', fontSize: 14, color: '#050505', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' as const }}
                          onMouseEnter={e => { e.currentTarget.style.background = '#f0f2f5' }}
                          onMouseLeave={e => { e.currentTarget.style.background = 'none' }}>
                          <Flag size={16} color="#65676b" /> Report profile
                        </button>
                        <button
                          onClick={() => { setMoreMenuOpen(false); blocked ? handleUnblock() : setConfirmBlock(true) }}
                          style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px 14px', fontSize: 14, color: blocked ? '#050505' : '#dc2626', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' as const }}
                          onMouseEnter={e => { e.currentTarget.style.background = blocked ? '#f0f2f5' : '#fef2f2' }}
                          onMouseLeave={e => { e.currentTarget.style.background = 'none' }}>
                          <Ban size={16} color={blocked ? '#65676b' : '#dc2626'} /> {blocked ? 'Unblock' : 'Block'}
                        </button>
                      </div>
                    )}
                  </div>
          ...
          {activeTab === 'posts' ? (
            <>
              {isOwnProfile && currentUser && (
                reshareTarget ? (
                  <CreatePost user={currentUser} avatarColor={currentProfile?.avatar_color ?? 0} avatarUrl={currentProfile?.avatar_url ?? null}
                        ...
                        {visiblePosts.map((post, i) => (
                          <div key={post.id} style={{ borderTop: i > 0 ? '1px solid #f0f2f5' : 'none' }}>
                            <PostCard post={post} currentUserId={currentUser?.id ?? null}
                              ...
                              onPostShared={p => setPosts(prev => [p, ...prev])}
