...
import type { Profile, PostWithProfile, PostCategory } from '../../_types'
...
import UserCard from '../../_components/social/user/UserCard'
...
const MAIN_NAV_DEF = [
  ...
  { icon: <Bookmark size={20} />, labelKey: 'nav_saved' as const, href: '/saved', active: false, bg: '#f59e0b' },
  { icon: <Clock size={20} />, labelKey: 'nav_recent' as const, href: '/recents', active: false, bg: '#64748b' },
]
...
function FeedPage() {
  usePageView('feed')
  useEffect(() => { document.title = 'Feed — CivilAxis' }, [])
  ...
  useEffect(() => {
    async function init() {
      // Seed from the cached session first (reads localStorage, near-instant) so the logged-in
      // nav renders without waiting on the getUser() network round-trip — avoids the F5 flash.
      const { data: { session } } = await supabase.auth.getSession()
      if (session?.user) {
        setUser(session.user); userRef.current = session.user
        // Seed the cached profile too, so the avatar image renders on first paint instead of
        // flashing the initials-circle fallback until the profiles fetch returns.
        try {
      ...
      if (authUser) {
        ...
        try { if (profileData) localStorage.setItem(`civilbase_profile_${authUser.id}`, JSON.stringify(profileData)) } catch {}

        // Badge counts (notifications, friend requests, messages) are self-seeded and kept live
        // by their respective dropdown components — no parent seeding/subscription needed here.
        ...
        const blocked = await getBlockedIds(authUser.id)
      ...
      const fetchedPosts = await attachResharedPosts((data as PostWithProfile[]) ?? [])
  ...
  const loadMore = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .limit(20)
    const more = await attachResharedPosts((data as PostWithProfile[]) ?? [])
    setPosts(prev => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f2f5', display: 'flex', flexDirection: 'column' }}>
      ...
      {user && (() => {
        let expandedIdx = 0
        let minimizedIdx = 0
        return chat.openChats.map((c) => {
          const isMin = chat.minimizedChats.has(c.convId)
          const idx = isMin ? minimizedIdx++ : expandedIdx++
          return <ChatBox key={c.convId} userId={user.id} {...chat.chatBoxProps(c, idx)} />
        })
      })()}
      ...
      <div style={{
        ...
        <div style={{ position: 'sticky', top: 68, display: 'flex', flexDirection: 'column', gap: 2, overflowY: 'auto', maxHeight: 'calc(100vh - 80px)' }}>
          ...
          {user && (
            <a href={profileHref}
              ...
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: AVATAR_COLORS[avatarColor], display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 800, color: '#fff', flexShrink: 0, overflow: 'hidden' }}>
                {profile?.avatar_url
                  ? <img src={profile.avatar_url} alt={displayName[0].toUpperCase()} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          ...
          <button onClick={() => { if (!user) { setSignInPromptAction('comment'); return } setShowFeedback(true) }}
            ...
            <div>
              <div style={{ fontSize: 15, fontWeight: 600, color: '#050505' }}>{t('nav_btn_feedback_label')}</div>
              <div style={{ fontSize: 11, color: '#65676b' }}>{t('fb_sub_short')}</div>
            </div>
          </button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 590, width: '100%', justifySelf: 'center' }}>
          ...
          {user ? (
            reshareTarget ? (
              <CreatePost user={user} avatarColor={avatarColor} avatarUrl={profile?.avatar_url ?? null} displayName={displayName}
                onPostCreated={handlePostCreated} resharePost={reshareTarget} onCancelReshare={() => setReshareTarget(null)} />
            ) : (
              <CreatePost user={user} avatarColor={avatarColor} avatarUrl={profile?.avatar_url ?? null} displayName={displayName} onPostCreated={handlePostCreated} />
            )
          ) : (
        ...
        <div style={{ position: 'sticky', top: 68, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ padding: '0 4px' }}>
            <div style={{ fontSize: 12, color: '#bcc0c4', lineHeight: 2 }}>
              CivilAxis · Community · Feed
            </div>
            <div style={{ fontSize: 12, color: '#bcc0c4' }}>© 2025 CivilAxis</div>
          </div>
        </div>
      </div>
    </div>
