...
export default function NotificationDropdown({ userId, unreadCount, onUnreadChange, onOpenPost }: Props) {
  ...
  async function load() {
    ...
    // Fetch post previews for notifications that have a post_id
    ...
    if (postIds.length > 0) {
      const { data: posts } = await supabase
        .from('posts').select('id, body, media_url').in('id', postIds)
  ...
  async function handleClick(n: Notification) {
    ...
    if (n.type === 'verify_revoked') {
      setRevokeNotif(n)
      return
    }
    if (n.type === 'verify_rejected') {
      ...
      <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 20px 60px rgba(0,0,0,0.3)', width: '100%', maxWidth: 520, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ background: warningNotif.type === 'post_deleted' ? 'linear-gradient(135deg, #dc2626, #b91c1c)' : 'linear-gradient(135deg, #d97706, #b45309)', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.8)', marginTop: 2 }}>From Civil Base Admin</div>
          </div>
        </div>
        ...
        <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ fontSize: 15, color: '#0f172a', lineHeight: 1.65 }}>
            {warningNotif.type === 'post_deleted' ? (
              <>You have received <strong>1 warning</strong> from <strong>Civil Base Admin</strong> because your post was removed for violating our community standards.</>
            ) : (
              <>You have received <strong>1 warning</strong> from <strong>Civil Base Admin</strong> because your post was flagged as potentially violating our community standards.</>
            )}
          </div>
          ...
          <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ fontSize: 13, color: '#475569', lineHeight: 1.6 }}>
              Receiving <strong>3 warnings</strong> will result in your account being <strong style={{ color: '#dc2626' }}>permanently banned</strong> from Civil Base. Please review our community guidelines and ensure your posts comply with our standards.
            </div>
            <div style={{ fontSize: 13, color: '#475569', lineHeight: 1.6 }}>
      ...
      <div style={{ background: '#fff', borderRadius: 20, boxShadow: '0 24px 64px rgba(0,0,0,0.35)', width: '100%', maxWidth: 520, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #0369a1, #0284c7, #38bdf8)', padding: '28px 28px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, textAlign: 'center' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.85)', marginTop: 4 }}>Welcome to the Civil Base professional community</div>
          </div>
        </div>
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 9000, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#fff', borderRadius: 20, boxShadow: '0 24px 64px rgba(0,0,0,0.35)', width: '100%', maxWidth: 520, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #dc2626, #b91c1c)', padding: '28px 28px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, textAlign: 'center' as const }}>
          ...
          <div>
            <div style={{ fontSize: 22, fontWeight: 900, color: '#fff', letterSpacing: -0.3 }}>Verification revoked</div>
            <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.8)', marginTop: 4 }}>Your professional badge has been removed</div>
          </div>
        </div>
        ...
        <div style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Apology message */}
          <div style={{ fontSize: 15, color: '#0f172a', lineHeight: 1.65 }}>
            We're sorry to inform you that your professional verification has been <strong>revoked by Civil Base Admin</strong>. Your verified badge is no longer visible on your profile and posts.
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              { icon: '📋', title: 'Review the guidelines', desc: 'Make sure your activity aligns with Civil Base community standards.' },
        ...
        <div style={{ position: 'fixed', top: 60, right: 8, width: 380, maxHeight: '82vh', background: '#fff', borderRadius: 12, boxShadow: '0 8px 40px rgba(0,0,0,0.22)', border: '1px solid #e4e6eb', zIndex: 500, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            ...
            {displayed.map(n => {
              ...
              const actor = n.actor as any
              const isAdminAction = n.type === 'post_deleted' || n.type === 'post_warned' || n.type === 'verify_approved' || n.type === 'verify_rejected' || n.type === 'verify_revoked'
              const actorName = isAdminAction ? 'Civil Base Admin' : (actor?.display_name || actor?.full_name || actor?.username || 'Someone')
              const initial = actorName[0].toUpperCase()
              return (
                <div key={n.id}
                  onClick={() => handleClick(n)}
                  style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 14px', cursor: 'pointer', background: isAdminAction ? (n.type === 'verify_approved' ? (n.read ? '#f0f9ff' : '#e0f2fe') : (n.type === 'verify_rejected' || n.type === 'verify_revoked') ? (n.read ? '#fff1f2' : '#ffe4e6') : (n.read ? '#fffbeb' : '#fff8ed')) : (n.read ? '#fff' : '#f0f7ff'), borderBottom: '1px solid #f0f2f5', position: 'relative', borderLeft: isAdminAction ? `3px solid ${n.type === 'post_deleted' || n.type === 'verify_rejected' || n.type === 'verify_revoked' ? '#ef4444' : n.type === 'post_warned' ? '#f59e0b' : '#0369a1'}` : 'none' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = isAdminAction ? (n.type === 'verify_approved' ? '#bae6fd' : (n.type === 'verify_rejected' || n.type === 'verify_revoked') ? '#fecdd3' : '#fff3d6') : (n.read ? '#f8fafc' : '#e7f0ff') }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = isAdminAction ? (n.type === 'verify_approved' ? (n.read ? '#f0f9ff' : '#e0f2fe') : (n.type === 'verify_rejected' || n.type === 'verify_revoked') ? (n.read ? '#fff1f2' : '#ffe4e6') : (n.read ? '#fffbeb' : '#fff8ed')) : (n.read ? '#fff' : '#f0f7ff') }}>

                  {/* Avatar + badge */}
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    {isAdminAction ? (
                      <div style={{ width: 44, height: 44, borderRadius: '50%', background: n.type === 'post_deleted' ? 'linear-gradient(135deg, #ef4444, #b91c1c)' : n.type === 'post_warned' ? 'linear-gradient(135deg, #f59e0b, #d97706)' : (n.type === 'verify_revoked' || n.type === 'verify_rejected') ? 'linear-gradient(135deg, #ef4444, #b91c1c)' : 'linear-gradient(135deg, #0369a1, #0284c7)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>
                        {n.type === 'post_deleted' ? '🚫' : n.type === 'post_warned' ? '⚠️' : (n.type === 'verify_revoked' || n.type === 'verify_rejected') ? '🛡️' : '🛡️'}
                      </div>
                    ) : (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    {isAdminAction ? (
                      <div style={{ fontSize: 13, lineHeight: 1.5, color: n.type === 'verify_approved' || n.type === 'verify_rejected' ? '#0c4a6e' : '#92400e' }}>
                        <strong style={{ fontWeight: 700, color: n.type === 'post_deleted' ? '#dc2626' : n.type === 'post_warned' ? '#b45309' : n.type === 'verify_approved' ? '#0369a1' : '#dc2626' }}>
                          {n.type === 'post_deleted' ? '🚫 Post removed' : n.type === 'post_warned' ? '⚠️ Warning issued' : n.type === 'verify_approved' ? '🛡️ Verification approved' : n.type === 'verify_revoked' ? '🛡️ Verification revoked' : '🛡️ Verification rejected'}
                        </strong>
                        <div style={{ fontSize: 12, marginTop: 2, fontWeight: n.read ? 400 : 600, color: n.type === 'verify_approved' ? '#0369a1' : (n.type === 'verify_rejected' || n.type === 'verify_revoked') ? '#dc2626' : '#78350f' }}>
                          {n.message || notifMsg(n.type)}
                        </div>
                        {(n.type === 'post_deleted' || n.type === 'post_warned') && (
                          <div style={{ fontSize: 11, color: '#b45309', marginTop: 3, fontWeight: 600 }}>Tap to view full notice →</div>
                        )}
                      </div>
                    ) : (
