...
import type { LectureMindMap, MindMapNode } from '../types'
...
// ── Node row (read-only, collapsible) ────────────────────────────────────────
...
function LectureCard({ entry, sectionId, onSave }: {
  ...
  // Keep ref in sync so the effect can read current dirty state
  useEffect(() => { dirtyRef.current = dirty }, [dirty])

  // Sync from DB only when not currently editing
  const mapUpdatedAt = entry.map?.updated_at
  useEffect(() => {
    if (dirtyRef.current) return // don't overwrite in-progress edits
    ...
    <div style={{ border: `1px solid ${open ? '#bfdbfe' : '#e2e8f0'}`, borderRadius: 10, overflow: 'hidden', marginBottom: 8, transition: 'border-color 0.15s' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 12px', background: open ? '#f0f7ff' : '#fff', cursor: 'pointer', transition: 'background 0.15s, box-shadow 0.15s' }}
        ...
        <button onClick={e => { e.stopPropagation(); window.open(entry.pdfUrl, '_blank') }}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', padding: '2px', borderRadius: 5, display: 'flex', flexShrink: 0 }} title="Open PDF">
...
// ── Bulk add textarea ─────────────────────────────────────────────────────────
// Format: blocks separated by blank lines, each block's first line = PDF name match
function BulkAdd({ lectureMaps, sectionId, onSave, onClose }: {
  ...
  async function handleSave() {
    ...
    for (let blockIdx = 0; blockIdx < blocks.length; blockIdx++) {
      ...
      // 1. Extract lecture number from content lines (e.g. "- Lec 3-10" → lec 3)
      ...
      // 2. Fall back: match by block position (block 0 → lec 1, etc.)
      if (!match) {
        match = available[0] // next unused in order
      }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderBottom: '1px solid #e2e8f0', background: '#fff' }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: '#1e293b', flex: 1 }}>Bulk Add</span>
        <span style={{ fontSize: 11, color: '#94a3b8' }}>Separate lectures with <code>===</code>. Matched by <code>Lec N-page</code> numbers in content, then by order.</span>
        <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', fontSize: 12, padding: '3px 8px', borderRadius: 6 }}>✕ Cancel</button>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {log.length > 0 && (
          <div style={{ padding: '10px 14px', borderTop: '1px solid #e2e8f0', background: '#f8fafc', fontSize: 12, lineHeight: 1.8 }}>
            ...
            <button onClick={onClose} style={{ marginTop: 8, padding: '3px 12px', borderRadius: 6, background: '#3b82f6', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 12 }}>Done</button>
          </div>
        )}
      </div>
    </div>
...
interface Props {
  sectionId: string
  lectureMaps: LectureMindMap[]
  pdfOrder: string[] // ordered list of pdfIds from the lecture notes FilePanel
  onSave: (sectionId: string, pdfId: string, title: string, nodes: MindMapNode[]) => Promise<void>
...
export default function MindMapPanel({ sectionId, lectureMaps, pdfOrder, onSave, onDelete }: Props) {
  ...
  // Always reflect the latest lectureMaps order (which already has PDF order applied)
  useEffect(() => {
      ...
      {selectMode ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderBottom: '1px solid #e2e8f0', background: '#fef2f2' }}>
          ...
          <button onClick={() => setSelected(new Set(lectureMaps.filter(e => e.map).map(e => e.pdfId)))}
            style={{ fontSize: 12, color: '#64748b', background: 'none', border: 'none', cursor: 'pointer' }}>Select all with data</button>
          <button onClick={handleDelete} disabled={!selected.size || deleting}
            ...
            <Trash2 size={12} /> {deleting ? 'Clearing…' : 'Clear data'}
          </button>
          <button onClick={() => { setSelectMode(false); setSelected(new Set()) }}
            style={{ fontSize: 12, color: '#64748b', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px' }}>Cancel</button>
        </div>
      ) : (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderBottom: '1px solid #e2e8f0', background: '#fff' }}>
          <span style={{ fontSize: 15 }}>🗺️</span>
          <span style={{ fontSize: 13, fontWeight: 600, color: '#1e293b' }}>Mind Map</span>
          ...
          <button onClick={applyLectureOrder} disabled={!pdfOrder.length}
            ...
            title="Sort mindmap cards to match lecture notes order">
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
        {lectureMaps.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: '#94a3b8' }}>
            ...
            <p style={{ fontSize: 12, marginTop: 4 }}>Upload PDFs to the Lecture Notes section first.</p>
          </div>
        ) : (
          localMaps.map((entry) => (
