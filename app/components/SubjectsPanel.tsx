...
import type { Subject, RenameState } from '../types'

// Auto-assign a stable color per category name
...
export default function SubjectsPanel({
  ...
  // Build category list from subjects
  ...
  // Only group by category when a filter is active; otherwise show flat sorted list
      ...
      <div key={s.id} tabIndex={0}
        ...
        {renaming?.id === s.id ? (
          <div style={{ flex: 1, minWidth: 0 }} onClick={e => e.stopPropagation()}>
            ...
            <input value={renaming!.code ?? ''}
              ...
              onKeyDown={onRenameKey} onBlur={onRenameBlur} placeholder="Code"
            ...
            <input value={renaming!.category ?? ''}
              ...
              onKeyDown={onRenameKey} onBlur={onRenameBlur} placeholder="Category (e.g. Steel)"
        ...
        {!selectMode && renaming?.id !== s.id && (
          <div style={{ display: 'flex', gap: '2px', flexShrink: 0 }}>
            <button onClick={e => onOpenChat(e, s)} title="Ask AI"
    ...
    <div style={{ flex: 1, borderRight: '1px solid #e2e8f0', background: 'white', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '10px 12px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
          <button onClick={onToggleSelectMode} title="Select mode"
          ...
          <button onClick={() => { setShowFilter(f => !f); if (showFilter) setFilterCategory(null) }} title="Filter by category"
      ...
      {/* Category filter chips — only shown when toggled */}
      ...
      {showAddSubject && (
        <div style={{ padding: '10px 12px', borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
          ...
          <input placeholder="Code (e.g. CE5509)" value={newCode} onChange={e => setNewCode(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && handleAdd()}
            style={{ width: '100%', padding: '6px 8px', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '13px', marginBottom: '6px', boxSizing: 'border-box' }} />
          <input placeholder="Category (e.g. Steel)" value={newCategory} onChange={e => setNewCategory(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && handleAdd()}
            style={{ width: '100%', padding: '6px 8px', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '13px', marginBottom: '6px', boxSizing: 'border-box' }} />
      ...
      {/* Subject list — flat sorted, category header only when filtering */}
      <div style={{ overflowY: 'auto', flex: 1 }}>
        ...
        {subjects.length === 0 && (
          <div style={{ padding: '40px 16px', textAlign: 'center', color: '#94a3b8', fontSize: 13 }}>No subjects yet</div>
        )}
      </div>
    </div>
  )
