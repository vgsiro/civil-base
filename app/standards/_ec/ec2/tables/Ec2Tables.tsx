...
export function ConcretePropertiesTable() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <SectionHeader
        title="Concrete Design Properties"
        subtitle="EN 1992-1-1 Table 3.1 · γC = 1.50 · fyk = 500 MPa"
      />

      {/* Main strength table */}
      <div>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Strength & Elastic Properties</div>
        <div style={{ overflowX: 'auto' }}>
          <Table>
            <thead>
              <tr>
                <th style={{ ...TH, textAlign: 'left', minWidth: 90 }}>Grade</th>
            ...
            <tbody>
              {CONCRETE_GRADES.map((g, i) => (
                <TR key={g.grade} stripe={i % 2 !== 0}>
                  <td style={{ ...TD, fontWeight: 700, color: '#10b981', textAlign: 'left' }}>{g.grade}</td>
                  <td style={TDN}>{g.fck}</td>
                  <td style={TDN}>{g.fck_cube}</td>
                  <td style={TDN}>{g.fcm}</td>
                  <td style={TDN}>{g.fctm.toFixed(2)}</td>
                  <td style={TDN}>{fctk05(g.fctm).toFixed(2)}</td>
                  <td style={TDN}>{fctk95(g.fctm).toFixed(2)}</td>
                  <td style={TDN}>{g.fctd.toFixed(2)}</td>
                  <td style={TDN}>{g.Ecm.toLocaleString()}</td>
                </TR>
              ))}
            </tbody>
          </Table>
      ...
      {/* Design values table */}
      <div>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Design Compressive Strength f<sub>cd</sub> & Min. Reinforcement Ratios</div>
        <div style={{ overflowX: 'auto' }}>
          <Table>
            <thead>
              <tr>
            ...
            <tbody>
              {CONCRETE_GRADES.map((g, i) => (
                <TR key={g.grade} stripe={i % 2 !== 0}>
                  <td style={{ ...TD, fontWeight: 700, color: '#10b981', textAlign: 'left' }}>{g.grade}</td>
                  <td style={TDN}>{g.fcd_100.toFixed(2)}</td>
                  <td style={TDN}>{g.fcd_085.toFixed(2)}</td>
                  <td style={TDN}>{g.rho_min.toFixed(3)}</td>
                  <td style={TDN}>{g.rw_min.toFixed(3)}</td>
                </TR>
              ))}
            </tbody>
          </Table>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#166534' }}>
          <strong>Formulas (EN 1992-1-1 §3.1.2, §3.1.6)</strong>
        ...
        <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#92400e' }}>
          <strong>Min. reinforcement ratios (EN 1992-1-1 §9.2.1.1, §9.2.2)</strong>
        ...
        <div style={{ fontSize: 11, color: '#64748b' }}>
          General: density ≈ 2500 kg/m³ · unit weight ≈ 25.0 kN/m³ · Poisson's ratio ν = 0.2 (uncracked) / 0.0 (cracked) · thermal expansion 10×10⁻⁶ /°K
        </div>
      </div>
    </div>
  )
...
const ANCHORAGE_DATA: { grade: string; g_str: number; g_bent: number; p_str: number; p_bent: number }[] = [
...
const LAP_DATA: { grade: string; g_str: number; g_bent: number; p_str: number; p_bent: number }[] = [
...
export function AnchorageLapTable() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#166534' }}>
          <strong>Good bond conditions (η₁ = 1.0)</strong>
          <ul style={{ margin: '4px 0 0 0', paddingLeft: 16, lineHeight: 1.7 }}>
            <li>Vertical / near-vertical bars (45° ≤ α ≤ 90°)</li>
            <li>Bars ≤ 250 mm from formwork base (h ≤ 600 mm)</li>
            <li>Bars ≥ 300 mm from free surface (h &gt; 600 mm)</li>
          </ul>
        </div>
        <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#9a3412' }}>
          <strong>Poor bond conditions (η₁ = 0.7)</strong>
          <ul style={{ margin: '4px 0 0 0', paddingLeft: 16, lineHeight: 1.7 }}>
            <li>All other positions</li>
            <li>Slip-formed structures (unless good conditions demonstrated)</li>
          </ul>
        </div>
      </div>
      ...
      <div>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          Design Anchorage Length l<sub>bd</sub> — multiples of Φ
        </div>
        <div style={{ overflowX: 'auto' }}>
          <Table>
            <thead>
              <tr>
                <th style={{ ...TH, textAlign: 'left', minWidth: 90 }} rowSpan={2}>Concrete</th>
                <th style={{ ...TH, borderBottom: '1px solid #e2e8f0' }} colSpan={2}>Good bond (η₁ = 1.0)</th>
                <th style={{ ...TH, borderBottom: '1px solid #e2e8f0' }} colSpan={2}>Poor bond (η₁ = 0.7)</th>
              </tr>
              <tr>
                {colH('Straight / compression')}
                {colH('Bent bars (cd > 3Φ)')}
                {colH('Straight / compression')}
                {colH('Bent bars (cd > 3Φ)')}
              </tr>
            </thead>
            <tbody>
              {ANCHORAGE_DATA.map((r, i) => (
                <TR key={r.grade} stripe={i % 2 !== 0}>
                  <td style={{ ...TD, fontWeight: 700, color: '#10b981', textAlign: 'left' }}>{r.grade}</td>
                  <td style={{ ...TDN, color: '#1d4ed8' }}>{r.g_str}Φ</td>
                  <td style={{ ...TDN, color: '#1d4ed8' }}>{r.g_bent}Φ</td>
                  <td style={{ ...TDN, color: '#dc2626' }}>{r.p_str}Φ</td>
                  <td style={{ ...TDN, color: '#dc2626' }}>{r.p_bent}Φ</td>
                </TR>
              ))}
            </tbody>
          </Table>
      ...
      <div>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          Design Lap Length l<sub>0</sub> — multiples of Φ &nbsp;<span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(α₆ = 1.5 — &gt;50% bars lapped in one section)</span>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <Table>
            <thead>
      ...
      {/* Minimum lengths */}
      <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#92400e' }}>
        <strong>Minimum lengths</strong>
        <div style={{ marginTop: 4, fontFamily: 'ui-monospace, monospace', lineHeight: 1.8 }}>
          <div>Tension anchorage: max(0.3 · l<sub>b,rqd</sub> , 10Φ , 100 mm)</div>
          <div>Compression anchorage: max(0.6 · l<sub>b,rqd</sub> , 10Φ , 100 mm)</div>
          <div>Lap length: max(0.3 · α₆ · l<sub>b,rqd</sub> , 15Φ , 200 mm)</div>
        </div>
      </div>

      {/* Alpha table */}
      <div>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>α Coefficients</div>
        <Table>
          <thead>
            <tr>
              <th style={{ ...TH, textAlign: 'center', minWidth: 48 }}>Coeff.</th>
              <th style={{ ...TH, textAlign: 'left' }}>Application</th>
              <th style={{ ...TH, textAlign: 'left', minWidth: 320 }}>Values</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#475569' }}>
        <strong style={{ color: '#1e293b' }}>Lap detailing requirements</strong>
        <ul style={{ margin: '6px 0 0 0', paddingLeft: 16, lineHeight: 1.8 }}>
          <li>Clear distance between lapped bars ≤ 4Φ or 50 mm</li>
          <li>Distance between adjacent laps ≥ 0.3 l₀</li>
          <li>Clear distance for adjacent lapped bars ≥ 2Φ or 20 mm</li>
          <li>Spacing between transverse reinforcement ≤ 150 mm</li>
          <li>If Φ ≥ 20 mm: Σ A<sub>st</sub> ≥ A<sub>s</sub> of one lapped bar</li>
        </ul>
      </div>
    </div>
  )
...
export function ReinforcementQuantityTable() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <SectionHeader
        title="Reinforcement Quantity Tables"
      ...
      <div>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          Area of Distributed Reinforcement A<sub>s</sub> (cm²/m)
        </div>
        <div style={{ overflowX: 'auto' }}>
          <Table>
            <thead>
              <tr>
                <th style={{ ...TH, textAlign: 'left', minWidth: 80 }}>Spacing (mm)</th>
                {DIAMETERS.map(d => (
      ...
      <div>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          Weight of Distributed Reinforcement (kg/m²)
        </div>
        <div style={{ overflowX: 'auto' }}>
          <Table>
            <thead>
      ...
      <div>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          Area of Individual Bars A<sub>s</sub> (cm²) — n bars
        </div>
        <div style={{ overflowX: 'auto' }}>
          <Table>
            <thead>
      ...
      <div>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          Weight of Individual Bars (kg/m) — n bars
        </div>
        <div style={{ overflowX: 'auto' }}>
          <Table>
            <thead>
