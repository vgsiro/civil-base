    ...
    >
      ...
      {rect && (
        <span style={{
          ...
          borderRadius: 7, padding: '8px 11px', width: 300, whiteSpace: 'normal',
          boxShadow: '0 4px 16px rgba(0,0,0,0.18)', pointerEvents: 'none',
        }}>
          {typeof text === 'string' ? <TexInline s={text} /> : text}
          <span style={{ position: 'absolute', top: '100%', left: '50%', transform: 'translateX(-50%)', width: 0, height: 0, borderLeft: '6px solid transparent', borderRight: '6px solid transparent', borderTop: '6px solid #1e293b' }} />
        </span>
      )}
    </span>
  )
...
export function DetailsSection({ id, children }: { id?: string; children: React.ReactNode }) {
  const [open, setOpen] = useState(false)
  return (
    <div id={id} style={{ border: '1px solid #e2e8f0', borderRadius: 8, overflow: 'hidden' }}>
      <button onClick={() => setOpen(o => !o)}
        style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 14px', background: open ? '#f8fafc' : '#fff', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: '#1e293b' }}>
        <span>Details</span>
        <span style={{ fontSize: 16, lineHeight: 1, transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>▾</span>
      </button>
      {open && (
...
export function CalcStep({ label, formula, formulaNode, substitution, unit, note, tableRef }: {
  ...
  return (
    <div style={DETAILS_STEP}>
      ...
      {note && <div style={{ fontSize: 12, color: '#b45309', marginTop: 4, lineHeight: 1.5 }}>{note}</div>}
    </div>
  )
}
